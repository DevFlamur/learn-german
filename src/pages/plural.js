...
import words from "../data/plural/allchapters.json"
import WordInput from "../components/wordinput"
import Speech from "speak-tts"
import PlayCircleOutlineIcon from "@material-ui/icons/PlayCircleOutline"
import Grid from "@material-ui/core/Grid"

function Plural({ location }) {
  let chapter = 0
  ...
  const checkQuestion = (answer, answerCorrect) => {
    ...
    if (answer === answerCorrect && !wasWrongBefore) {
      ...
      aq.push({
        ...
        isCorrect: true,
        questionText: words[chapter][ca[0].index].word,
      ...
      setIsWrongText(words[chapter][ca[0].index].correct)
      if (!wasWrongBefore) {
        aq.push({
          answered: answer,
          correct: answerCorrect,
  ...
  const handleClick = () => {
    speech
      .speak({
        text:
          getArticleText(words[chapter][currentQuestion[0].index].article) +
          " " +
          words[chapter][currentQuestion[0].index].word,
      })
      .then(() => {
        console.log("Success !")
      })
  ...
  if (typeof location.state !== "undefined" && location.state) {
        ...
        >
          ...
          <Grid container spacing={0}>
            <Grid item xs={11} style={{ height: "40px", textAlign: "center" }}>
              <h3>
                {getArticleText(
                  words[chapter][currentQuestion[0].index].article
                )}{" "}
                {words[chapter][currentQuestion[0].index].word}
              </h3>
            </Grid>
          ...
          <br />
          <WordInput setAnswerText={setAnswer} value={answerText} />
          <br /> <br /> <br />
          <CheckButton
    ...
    return (
      <>
        ...
        <input type="button" className="backBtn" value="Zurück" />
      </>
    )
  }
}
